...
func (self *GetPropertyService) Execute(propertyID uint64, userIdentity *string) (*entities.Property, error) {
	...
	getPropertyTransaction := self.Database.Preload(clause.Associations).Preload("Visits").Find(&property, propertyID).Where("deleted_at IS NULL and is_sold IS NULL").First(&property)
